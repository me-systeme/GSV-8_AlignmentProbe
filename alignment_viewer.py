...
Acquires strain-gauge data from a ME-Systeme GSV-8 device (via MEGSV86x64.dll)
and visualizes bending strain on two orthogonal planes with color-coded
alignment classes.

Features:
- Live data capture (8 channels @ 50 Hz)
...
COM_PORT = int(CONFIG["device"]["com_port"])
SAMPLE_FREQUENCY = float(CONFIG["device"]["sample_frequency"])
DLL_NAME = str(CONFIG["device"]["dll_name"])
...
if not dll_path.exists():
    raise FileNotFoundError(f"'{DLL_NAME}' was not found in {__file__}")

MEGSV = C.WinDLL(str(dll_path))
...
MEGSV.GSV86getDataRateRange.argtypes = [c_int, C.POINTER(c_double), C.POINTER(c_double)]
MEGSV.GSV86getDataRateRange.restype  = c_int
MEGSV.GSV86getFrequency.argtypes = [c_int]
MEGSV.GSV86getFrequency.restype  = c_double
MEGSV.GSV86setModeAfilterAuto.argtypes = [c_int, c_int]
MEGSV.GSV86setModeAfilterAuto.restype  = c_int
...
MEGSV.GSV86read.argtypes = [c_int, c_int, C.POINTER(c_double)]
MEGSV.GSV86read.restype  = c_int
MEGSV.GSV86readMultiple.argtypes = [c_int, c_int, C.POINTER(c_double), c_int,
                                    C.POINTER(c_int), C.POINTER(c_int)]
MEGSV.GSV86readMultiple.restype  = c_int
MEGSV.GSV86release.argtypes = [c_int]
MEGSV.GSV86release.restype  = c_int
MEGSV.GSV86getLastErrorText.argtypes = [c_int, C.c_char_p]
...
def check(retcode: int, func_name: str, com: int = COM_PORT):
    if retcode < 0:
        ...
        raise RuntimeError(f"{func_name} failed (rc={retcode})" + (f": {msg}" if msg else ""))


def init_device(com: int):
    print(f"Connecting to MEGSV86 on COM{com} ...")
    check(MEGSV.GSV86actExt(com), "GSV86actExt", com)

    MEGSV.GSV86stopTX(com)
    dmax = c_double(0.0); dmin = c_double(0.0)
    check(MEGSV.GSV86getDataRateRange(com, byref(dmax), byref(dmin)), "GSV86getDataRateRange", com)
    print(f"Allowed data rate range according to device: {dmin.value:.3f} .. {dmax.value:.3f} Hz")
    target = float(np.clip(SAMPLE_FREQUENCY, dmin.value, dmax.value))
    if abs(target - SAMPLE_FREQUENCY) > 1e-12:
        print(f"Requested frequency {SAMPLE_FREQUENCY} Hz invalid – use instead {target:.6g} Hz.")
    check(MEGSV.GSV86setFrequency(com, target), "GSV86setFrequency", com)
    MEGSV.GSV86setModeAfilterAuto(com, 1)
    check(MEGSV.GSV86startTX(com), "GSV86startTX", com)
    eff = MEGSV.GSV86getFrequency(com)
    print(f"Actual device frequency: {eff:.6g} Hz")
...
class BendingView(QtWidgets.QWidget):
    ...
    def _read_values(self):
        NUM_OBJS = len(CHANNELS)  # for GSV-8 typically 8
        COUNT = NUM_OBJS * self.mult_frames  # must be divisible by NUM_OBJS (Chan=0)
        out_arr = (c_double * COUNT)()
        valsread = c_int(0)
        errflags = c_int(0)

        rc = MEGSV.GSV86readMultiple(COM_PORT, 0, out_arr, COUNT, byref(valsread), byref(errflags))

        if rc < 0:
            # Clear device/DLL buffers only on errors
            try:
                MEGSV.GSV86clearDeviceBuf(COM_PORT)
                MEGSV.GSV86clearDLLbuffer(COM_PORT)
            except Exception:
                pass
            raise RuntimeError(f"GSV86readMultiple error: {dll_error_text(COM_PORT)}")


        if rc == 0 or valsread.value < NUM_OBJS:
            # GSV_OK: currently nothing in DLL buffer – return last valid frame
            self._empty_reads += 1
            return dict(self._last_vals)

        # We read values (GSV_TRUE). Extract last complete frame.
        self._empty_reads = 0
        n = valsread.value // NUM_OBJS
        start = (n - 1) * NUM_OBJS
        last_frame = [out_arr[start + i] for i in range(NUM_OBJS)]

        # Mapping: Obj1..Obj8 ≙ channels 1..8, order see documentation gsv86dll reference manual
        # https://www.me-systeme.de/produkte/elektronik/gsv-8/anleitungen/gsv86dll_referencemanual.pdf
        # (oldest first, we take the last frame)
        vals = {CHANNELS[i]: last_frame[i] for i in range(NUM_OBJS)}
        self._last_vals = dict(vals)

        # Optional: evaluate flags (bit0 saturation, bit1 range exceed)
        if errflags.value:
            # Here you could change a status LED or log a debug message
            pass
    ...
    def closeEvent(self, event):
        ...
        try:
            ...
            print(f"Note: release reported: {e}")
...
def main():
    ...
    # Central cleanup (runs for all exit paths)
    def cleanup():
        try:
